...
export class FlexmodelClient {
  ...
  private entityApis: Map<string, any> = new Map();

  constructor(apiClient: ApiClient, datasourceName: string) {
   ...
   * @param ApiClass API类构造函数
   * @returns API实例
   */
  getApi<T>(modelName: string, ApiClass: new (apiClient: ApiClient, datasourceName: string, modelName: string) => T): T {
